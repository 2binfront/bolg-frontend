...
<template>
    <div class="flex">
        <div class="mx font-italic text-20px cp underline article-brief" v-for="tag in articleStore.tags" :key="tag.id"
            @click="showSpecificArticle(tag)">
            <div v-if="!editing">
                {{ tag.name }}
            </div>
            <div v-else>
        ...
        <div v-if="editing" class="ml-8 mt"> new Tag: <input type="text" v-model="addT.name"
                @keyup.enter="handleAddT(addT.name)" /> </div>
    </div>
</template>
